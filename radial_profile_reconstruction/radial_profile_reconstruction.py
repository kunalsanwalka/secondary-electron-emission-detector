...
def parseArgs():
    ...
    parser.add_argument('-d', '--decimationFactor',
                        type=int,
                        metavar='Decimation factor for the SEE data',
                        default=100,
                        help='The factor by which to decimate the SEE data. This is done to make the fitting more accurate. Default is 100.')

    try:
        print('try statement')
        args = parser.parse_args()
        print('parser.parse_args succeeded?')
    ...
    if args.ref_shotnum == 0:
        ...
        print('No reference shot number specified, pulling from MDSplus.')

        try:
            tree = mds.Tree('wham', args.shotnum)
            args.ref_shotnum = tree.getNode('diag.shinethru.ref_shotnum').data()
            tree.close()
            print(f'Pulled reference shot number {args.ref_shotnum} from MDSplus.')
        except Exception as e:
            print(f'Error occurred while pulling reference shot number from MDSplus: {e}')

    return args

def get_raw_shinethru_data():
    ...
    # Remove negative values from the data and set them to 0. This will make the fitting more accurate
    shinethruData[shinethruData < 0] = 0
    ...
    if args.debug:
        ...
        digFreq = 1/(newTimeArr[1] - newTimeArr[0])
        print(f"Digitization frequency: {digFreq/1e6:.2f} MHz")
...
def get_shinethru_data():
    ...
    # Get the raw data from MDSplus
    impactParams, shinethruData, shinethruDataErr, timeArr = get_raw_shinethru_data()

    n_detectors = shinethruData.shape[0]
    n_times = (shinethruData.shape[1] // args.decimationFactor) * args.decimationFactor

    # Trim to a multiple of factor along time axis
    shinethruData, shinethruDataErr = shinethruData[:, :n_times], shinethruDataErr[:, :n_times]
    timeArr = timeArr[:n_times]

    # Reshape into groups along time axis
    # (n_detectors, n_times//decimationFactor, decimationFactor)
    # Done so that averaging/variance can be calculated in 1 line
    x_groups = shinethruData.reshape(n_detectors, -1, args.decimationFactor)
    y_groups = shinethruDataErr.reshape(n_detectors, -1, args.decimationFactor)

    if args.debug:
        print("shinethruData shape:", shinethruData.shape)
        print("x_groups shape:", x_groups.shape)

    # Weights = 1 / sigma^2
    w = 1.0 / y_groups**2
    w_sum = np.sum(w, axis=2, keepdims=True)

    # Weighted mean of x
    shinethruData = np.sum(w * x_groups, axis=2) / np.sum(w, axis=2)

    # Propagated error = [1 / sqrt(sum(1 / sigma^2))] = [1 / sqrt(sum(w))]
    # This is the new error bar due to the decimation (averaging) process (Poisson statistics)
    var_propagated = 1.0 / w_sum.squeeze(2)

    # Weighted sample variance of x within each group
    # This is the error bar due to the scatter of the data within each group (Bessel-corrected)
    residuals = x_groups - shinethruData[:, :, np.newaxis]        # deviation from weighted mean

    var_scatter = (np.sum(w * residuals**2, axis=2)
                   / ((args.decimationFactor - 1) * w_sum.squeeze(2)))  # Bessel-corrected
    if args.decimationFactor == 1:
        var_scatter = np.zeros_like(var_propagated)

    # Total error bar
    shinethruDataErr = np.sqrt(var_propagated + var_scatter)

    # New time array
    newTimeArr = timeArr[::args.decimationFactor]

    if args.debug:

        print("Impact parameters:", impactParams)
        print("Shinethru data shape after decimation:", shinethruData.shape)
        print("Shinethru data error shape after decimation:", shinethruDataErr.shape)
        print("Time array shape after decimation:", newTimeArr.shape)

        digFreq = 1/(newTimeArr[1] - newTimeArr[0])
        print(f"Digitization frequency after decimation: {digFreq/1e3:.2f} kHz")
        ...
        for i in range(shinethruData.shape[0]):
            ax.plot(newTimeArr*1e3, shinethruData[i], label=f'{i+1}')
...
def integrated_spline_function(xInput, mean=[0], limit=[0.15], knot1Pos=[0.1], knot2Pos=[0.14], knot1Val=[1], knot2Val=[0], centralVal=[0.5]):
    ...
    # Get the density on the 2D grid
    rGrid = np.sqrt(xGrid**2 + yGrid**2)
    nGrid = construct_spline_function(rGrid.ravel(), 
                                      mean=0, 
                                      limit=limit, 
                                      knot1Pos=knot1Pos, 
                                      knot2Pos=knot2Pos, 
                                      knot1Val=knot1Val, 
                                      knot2Val=knot2Val, 
                                      centralVal=centralVal).reshape(rGrid.shape)
...
def centralVal_histogram():

    try:
        data = np.load('/home/sanwalka/shinethru/radial_profile_reconstruction/centralVal_histogram_data.npz')
        centralVals = data['centralVals']

        centralVals = np.log10(centralVals)

        fig = plt.figure(figsize=(10, 6), tight_layout=True)
        ax = fig.add_subplot(111)

        ax.hist(centralVals, bins=10, color='blue', alpha=0.7, edgecolor='black')
        ax.axvline(18, color='k', 
                   linewidth = 5,
                   label=r'Fitter bounds')
        ...
        ax.set_xlabel(r'log$_{10}$(Central Density) [m$^{-3}$]')
        ax.set_ylabel('Number of fits')
        ax.set_title(r'n$_e$(r=0) values from multiple fits')
        ax.legend()
        ...
        print('File not found. Running the rest of the code to generate the data and histogram.')

    nMax, rMax, rOffset, impactParams = 1e20, 0.15, 0.01, get_detector_positions()

    # Number of fits to perform for the histogram
    numFits = 100

    # Array to store the model for each fit
    models = []
    errors = []

    for i in range(numFits):

        print(f'Performing fit {i+1}/{numFits}')

        hollowLineIntegratedDensities, hollowRadialProfileFine, rArr = synthetic_hollow_profile(nMax, rMax, rOffset, impactParams, noiseLevel=0.05)
        ...
        models.append(hollowBestModel)
        errors.append(hollowError)

    # Plot the histogram of the central density values
    centralVals = [10**model.centralVal.value for model in models]

    np.savez('/home/sanwalka/shinethru/radial_profile_reconstruction/centralVal_histogram_data.npz', centralVals=centralVals)

    fig = plt.figure(figsize=(10, 6), tight_layout=True)
    ax = fig.add_subplot(111)

    ax.hist(centralVals, bins=10, color='blue', alpha=0.7)
    ax.set_xlabel(r'Central density [$m^{-3}$]')
    ax.set_ylabel('Number of fits')
    ax.set_title('Histogram of central density values from multiple fits')
...
def noise_vs_centralVal():
    ...
    ax.axhline(y=10**20, color='k', linestyle='--', label='True value')
    ...
    ax.axhline(y=10**19, color='k', linestyle='-', label='True value hollow')
...
def effect_of_decimation():

    decimationArr = np.array([1, 2, 5, 10, 20, 50, 100])   # decimation factor

    # List to hold the best fit models for each decimation factor
    modelsArr = []

    # List to hold the raw data for each decimation factor for plotting later
    dataArr = []
    dataArrErr = []
    impactParams = None
    maxLimit = 0

    # Go over each decimation factor and fit the data
    for i in range(len(decimationArr)):

        decimationFactor = decimationArr[i]
        print(f'Fitting data with decimation factor: {decimationFactor}')

        # Change args with the decimation factor
        args.decimationFactor = decimationFactor

        # Load the real SEE data
        impactParams, shinethruData, shinethruDataErr, timeArr = get_shinethru_data()

        # Fit the data t t=8ms
        timeIdx = np.argmin(np.abs(timeArr - 8e-3))
        dataToFit = shinethruData[:, timeIdx]
        dataToFitErr = shinethruDataErr[:, timeIdx]
        dataArr.append(dataToFit)
        dataArrErr.append(dataToFitErr)

        bestModel, error = best_fit(0, dataToFit.reshape(-1, 1), impactParams, errorBars=dataToFitErr.reshape(-1, 1))
        modelsArr.append(bestModel)

        currLimit = bestModel.limit.value
        if currLimit > maxLimit:
            maxLimit = currLimit

    # Plot the best fit against the input data for each decimation factor
    fig = plt.figure(figsize=(15, 10), tight_layout=True)

    # Color each decimation factor
    cmap = plt.get_cmap('viridis', len(decimationArr)).colors
    ...
    for i in range(len(decimationArr)):
        ...
        # Line integrated data and fits
        ax1.errorbar(impactParams, dataArr[i], 
                     yerr=dataArrErr[i],
                     fmt='o',
                     ms=5,
                     color=cmap[i])
        ...
        ax1.plot(xArr, modelsArr[i](xArr),
                 label=f'{decimationFactor}', 
                 linewidth=3,
                 color=cmap[i])

        # Radial density profiles
        fittedModel = modelsArr[i]
...
def nbi_params_comparison():

    # Load the NBI current and voltage for the reference and plasma shots
    tree = mds.Tree('wham', args.shotnum)
    refShotnum = tree.getNode('diag.shinethru.ref_shotnum').data()
    refTree = mds.Tree('wham', refShotnum)

    # Plasma shot
    voltageNode = tree.getNode('nbi.v_beam')
    currentNode = tree.getNode('nbi.i_beam')
    nbiVPlasma = voltageNode.data() / 1e3
    nbiIPlasma = currentNode.data()
    timeArr = voltageNode.dim_of().data() * 1e3 # s to ms

    # Reference shot
    voltageNode = refTree.getNode('nbi.v_beam')
    currentNode = refTree.getNode('nbi.i_beam')
    nbiVRef = voltageNode.data() / 1e3
    nbiIRef = currentNode.data()

    # Clip data less than 5kV
    mask = nbiVPlasma >= 1
    nbiVPlasma = nbiVPlasma[mask]
    nbiIPlasma = nbiIPlasma[mask]
    nbiVRef = nbiVRef[mask]
    nbiIRef = nbiIRef[mask]
    timeArr = timeArr[mask]

    # % error
    voltageErr = 100 * np.abs((nbiVPlasma - nbiVRef)/nbiVPlasma)
    currentErr = 100 * np.abs((nbiIPlasma - nbiIRef)/nbiIPlasma)
    ...
    # % difference in NBI voltage
    ax3 = fig.add_subplot(223)
    # % difference in NBI current
    ax4 = fig.add_subplot(224)

    ax1.plot(timeArr, nbiVPlasma, label=f'Plasma {args.shotnum}', color='r')
    ax1.plot(timeArr, nbiVRef, label=f'Reference {refShotnum}', color='b')
    ax1.set_xlabel('Time [ms]')
    ax1.set_ylabel('Voltage [kV]')
    ax1.legend()

    ax2.plot(timeArr, nbiIPlasma, label=f'Plasma {args.shotnum}', color='r')
    ax2.plot(timeArr, nbiIRef, label=f'Reference {refShotnum}', color='b')
    ax2.set_xlabel('Time [ms]')
    ax2.set_ylabel('Current [A]')

    ax3.plot(timeArr, voltageErr, color='g')
    ax3.set_xlabel('Time [ms]')
    ax3.set_ylabel('Voltage Error [%]')
    ax3.set_ylim(0, 20)

    ax4.plot(timeArr, currentErr, color='g')
    ax4.set_xlabel('Time [ms]')
    ax4.set_ylabel('Current Error [%]')
    ax4.set_ylim(0, 20)

    # plt.show()
    plt.close()

    # Apply a 100x decimation to the data

    n_times = (len(nbiVPlasma) // args.decimationFactor) * args.decimationFactor

    # Trim to a multiple of factor along time axis
    nbiVPlasma, nbiIPlasma = nbiVPlasma[:n_times], nbiIPlasma[:n_times]
    nbiVRef, nbiIRef = nbiVRef[:n_times], nbiIRef[:n_times]
    timeArr = timeArr[:n_times]

    # Reshape into groups along time axis
    # (n_times//decimationFactor, decimationFactor)
    # Done so that averaging/variance can be calculated in 1 line
    nbiVPlasma = nbiVPlasma.reshape(-1, args.decimationFactor)
    nbiIPlasma = nbiIPlasma.reshape(-1, args.decimationFactor)
    nbiVRef = nbiVRef.reshape(-1, args.decimationFactor)
    nbiIRef = nbiIRef.reshape(-1, args.decimationFactor)

    nbiVPlasma = np.mean(nbiVPlasma, axis=1)
    nbiIPlasma = np.mean(nbiIPlasma, axis=1)
    nbiVRef = np.mean(nbiVRef, axis=1)
    nbiIRef = np.mean(nbiIRef, axis=1)
    
    # % error
    voltageErr = 100 * np.abs((nbiVPlasma - nbiVRef)/nbiVPlasma)
    currentErr = 100 * np.abs((nbiIPlasma - nbiIRef)/nbiIPlasma)

    timeArr = timeArr[::args.decimationFactor]
...
def fit_shot():
    ...
    timePointsFit = np.array([4, 6]) * 1e-3
...
if __name__ == '__main__':
    ...
    # Test the effect of decimation on the fitting
    # effect_of_decimation()

    # Histogram of the central density values from multiple fits
    # centralVal_histogram()

    # Compare the NBI parameters
    # nbi_params_comparison()
