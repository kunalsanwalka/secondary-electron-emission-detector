...
def get_detector_positions(args):
...
def synthetic_circular_profile(nMax, rMax, rOffset, impactParams, args):
...
def synthetic_flat_profile(nMax, rMax, rOffset, impactParams, args):
...
def synthetic_gaussian_profile(nMax, rMax, rOffset, impactParams, args):
...
def synthetic_hollow_profile(nMax, rMax, rOffset, impactParams, args):
...
def construct_spline_function(x, mean=0, limit=0.15, knotPos=0.1, knotVal=1, centralVal=0.5):

    # Make a spline function that goes through the points (0, centralVal), (knotPos, knotVal) and (limit, 0)
    knotPosArr = np.array([0, knotPos, limit])
    knotPosArr = np.sort(knotPosArr)
    knotValArr = np.array([centralVal, knotVal, 0])
...
@custom_model
def integrated_spline_function(x, mean=[0], limit=[0.15], knotPos=[0.1], knotVal=[1], centralVal=[0.5]):
    ...
    nArr = construct_spline_function(rArr, mean=0, limit=limit, knotPos=knotPos, knotVal=knotVal, centralVal=centralVal)
    ...
    # Go over each impact parameter and get the density along that detectors line of sight
    detectorDensities = np.zeros(shape=(len(x), len(xArr)))
    for i in range(len(x)):

        # y positions along the line of sight of the detector
        yPosArr = np.full_like(xArr, fill_value=x[i])
...
def best_fit(i, dataToFit, impactParams, errorBars=None):
    ...
    knotPosLimits = (0.01, 0.2)
    knotValLimits = (18, 20)        # log scale
    ...
    bounds = [meanLimits,               # mean
              limitLimits,              # limit
              knotPosLimits,            # knotPos
              knotValLimits,            # knotVal (log scale)
              centralValLimits]         # centralVal (log scale)

    def objective(params):

        mean, limit, knotPos, knotVal, centralVal = params
        ...
        model.mean = mean
        model.limit = limit
        model.knotPos = knotPos
        model.knotVal = knotVal
    ...
    model.knotPos.min, model.knotPos.max = knotPosLimits
    model.knotVal.min, model.knotVal.max = knotValLimits
...
def best_fit2(i, dataToFit, impactParams, errorBars=None):

    models, errors = fit_data(dataToFit[:, i], impactParams, errorBars=errorBars)
    ...
    if args.debug:
        ...
        print(f'Mean: {models[bestModelIndex].mean.value:.4f} m')
        print(f'Limit: {models[bestModelIndex].limit.value:.4f} m')
        print(f'Knot position: {models[bestModelIndex].knotPos.value:.4f} m')
        print(f'Knot value: {models[bestModelIndex].knotVal.value:.2e} m^-3')
        print(f'Central value: {models[bestModelIndex].centralVal.value:.2e} m^-3')
        print(f'Error: {errors[bestModelIndex]:.2e}')

        fig = plt.figure(figsize=(12, 8), tight_layout=True)
        ax = fig.add_subplot(111)

        # Plot the data
        ax.errorbar(impactParams, dataToFit[:, i], yerr=errorBars, label='Data points', color='red', fmt='o', capsize=5)
        # ax.scatter(impactParams, dataToFit[:, i], label='Data points', color='red')
        # Plot the best fit
        xArr = np.linspace(np.min(impactParams), np.max(impactParams), 100)
        ax.plot(xArr, models[bestModelIndex](xArr), label=f'Best fit (Error: {errors[bestModelIndex]:.2e})', color='blue', linewidth=3)

        ax.set_xlabel('r [m]')
        ax.set_ylabel(r'$\int n \cdot dl$ [m$^{-2}$]')
        ax.set_title('Best fit to the data')
        ax.legend()

        plt.show()

    return models[bestModelIndex], errors[bestModelIndex]

def fit_vs_input(model_type):

    nMax, rMax, rOffset, impactParams = 1e20, 0.15, 0.01, get_detector_positions(args)
...
def effect_of_fit_count():
    ...
    # Synthetic data
    circLineIntegratedDensities, circRadialProfileFine, rArr = synthetic_circular_profile(nMax, rMax, rOffset, impactParams, args)
    flatLineIntegratedDensities, flatRadialProfileFine, rArr = synthetic_flat_profile(nMax, rMax, rOffset, impactParams, args)
    gaussianLineIntegratedDensities, gaussianRadialProfileFine, rArr = synthetic_gaussian_profile(nMax, rMax, rOffset, impactParams, args)
    hollowLineIntegratedDensities, hollowRadialProfileFine, rArr = synthetic_hollow_profile(nMax, rMax, rOffset, impactParams, args)
...
def global_optimizer_testing():
    ...
    circBestModel, circError = best_fit(0, circLineIntegratedDensities.reshape(-1, 1), impactParams)
    flatBestModel, flatError = best_fit(0, flatLineIntegratedDensities.reshape(-1, 1), impactParams)
    gaussianBestModel, gaussianError = best_fit(0, gaussianLineIntegratedDensities.reshape(-1, 1), impactParams)
    hollowBestModel, hollowError = best_fit(0, hollowLineIntegratedDensities.reshape(-1, 1), impactParams)

    # Calculate the error against the radial profile itself
    fittedDensity = construct_spline_function(impactParams, 
                                              mean=circBestModel.mean.value, 
                                              limit=circBestModel.limit.value, 
                                              knotPos=circBestModel.knotPos.value, 
                                              knotVal=10**circBestModel.knotVal.value, 
    ...
    fittedDensity = construct_spline_function(impactParams, 
                                              ...
                                              knotPos=flatBestModel.knotPos.value, 
                                              knotVal=10**flatBestModel.knotVal.value, 
    ...
    fittedDensity = construct_spline_function(impactParams, 
                                              ...
                                              knotPos=gaussianBestModel.knotPos.value, 
                                              knotVal=10**gaussianBestModel.knotVal.value, 
    ...
    fittedDensity = construct_spline_function(impactParams, 
                                              ...
                                              knotPos=hollowBestModel.knotPos.value, 
                                              knotVal=10**hollowBestModel.knotVal.value, 
    ...
    fittedDensity = construct_spline_function(rArr, 
                                              ...
                                              limit=bestModel.limit.value, 
                                              knotPos=bestModel.knotPos.value, 
                                              knotVal=10**bestModel.knotVal.value, 
...
def fit_shot():
    ...
    for i in range(len(timePointsFit)):
        ...
        # Line integrated data and fits
        ax1.errorbar(impactParams, dataToFit[:, i], 
                     yerr=dataToFitErr[:, i],
                     fmt='o',
                     ms=10,
                     color=f'C{i}')
        ...
        fittedDensity = construct_spline_function(xArr, 
                                                  ...
                                                  limit=fittedModel.limit.value, 
                                                  knotPos=fittedModel.knotPos.value, 
                                                  knotVal=10**fittedModel.knotVal.value, 
        ...
        # Knot positions
        knotPosArr = np.array([0, fittedModel.knotPos.value, fittedModel.limit.value]) + fittedModel.mean.value
        knotValArr = np.array([10**fittedModel.centralVal.value, 10**fittedModel.knotVal.value, 0])
        ax2.scatter(knotPosArr, knotValArr,
                    color=f'C{i}', s=200, zorder=5)
...
if __name__ == '__main__':
    ...
    impactParams = get_detector_positions(args)

    # Make the synthetic circular profile
    nMax = 1e20
    rMax = 0.15
    rOffset = 0.05
    # lineIntegratedDensities, radialProfile, rArr = synthetic_circular_profile(nMax, rMax, rOffset, impactParams, args)
    # lineIntegratedDensities, radialProfile, rArr = synthetic_flat_profile(nMax, rMax, rOffset,impactParams, args)
    # lineIntegratedDensities, radialProfile, rArr = synthetic_gaussian_profile(nMax, rMax, rOffset,impactParams, args)
    # lineIntegratedDensities, radialProfile, rArr = synthetic_hollow_profile(nMax, rMax, rOffset,impactParams, args)

    # spline1 = construct_spline_function(np.linspace(np.min(impactParams)*1.5, np.max(impactParams)*1.5, 100), mean=0.03, limit=0.15, knotPos=0.1, knotVal=1, centralVal=0.5)
